...
import { toastMessageSetting } from './ToastMessageSetting';
...
function App() {
  ...
  const handleGeneratePassword = () => {
    if (!includeUpperCase && !includeLowerCase && !includeNumbers && !includeSymbols) {
      ...
      notify("Minimum password length is 10 characters", true)
    }
    else {
      let characterList = ""
      if (includeNumbers) {
      ...
      try {
        ...
        notify("Fail to generate password", true);
      }
    }
  }
  ...
  const notify = (message, hasError = false) => {
    ...
    if (hasError) {
      toast.error(message, toastMessageSetting);
    }
    else {
      toast(message, toastMessageSetting);
    }
  }
      ...
      <div className="container">
        <div className="generator">
          ...
          <button className="generator__btn">
            <i onClick={handleCopyPassword} className="far fa-clipboard">Copy to clipboard</i>
          </button>
